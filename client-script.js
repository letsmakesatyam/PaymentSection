...
fetch('https://raw.githubusercontent.com/letsmakesatyam/portfolio-seat-control/main/seats.json')
  .then(res => res.json())
  .then(data => {
  ...
  .catch(() => {
    console.warn("Seat control check failed. Assuming seats open.");
  });
...
document.getElementById('payBtn').addEventListener('click', () => {
  ...
  // ✅ Submit form to Basin when user confirms payment
  setTimeout(() => {
    document.getElementById('confirmPaidBtn').addEventListener('click', async () => {
      ...
      try {
        const response = await fetch('https://usebasin.com/f/e6079728f5c2', {
          method: 'POST',
          body: formData,
          headers: {
